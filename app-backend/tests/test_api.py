...
class TestMoveEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_create_move_success(self, client: AsyncClient, starting_fen: str, after_e4_fen: str):
        """
...
class TestMinimaxEndpoint:
    ...
    async def test_minimax_valid_fen(self, client: AsyncClient, starting_fen: str):
        ...
        response = await client.get(
            "/minimax",
            params={"fen": starting_fen, "depth": 2}
        )
    ...
    @pytest.mark.asyncio
    async def test_minimax_scholars_mate(self, client: AsyncClient, scholars_mate_fen: str):
        """GET /minimax for Scholar's Mate position should find Qxf7#."""
        response = await client.get(
            "/minimax",
            params={"fen": scholars_mate_fen, "depth": 3}
        )
    ...
    async def test_minimax_invalid_fen(self, client: AsyncClient):
        ...
        response = await client.get(
            "/minimax",
            params={"fen": "invalid_fen_string", "depth": 2}
        )
    ...
    async def test_minimax_checkmate_status(self, client: AsyncClient):
        ...
        response = await client.get(
            "/minimax",
            params={"fen": checkmate_fen, "depth": 1}
        )
...
class TestStatsEndpoint:
    ...
    async def test_stats_returns_move_statistics(self, client: AsyncClient):
        ...
        moves_to_create = [
            {"move": "e2e4", "end_fen": "rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq e3", "white": 1000, "black": 500, "draw": 300},
            {"move": "d2d4", "end_fen": "rnbqkbnr/pppppppp/8/8/3P4/8/PPP1PPPP/RNBQKBNR b KQkq d3", "white": 800, "black": 400, "draw": 200},
        ]
        
        for move in moves_to_create:
            await client.post("/moves/", json={
                "move": move["move"],
                "start_fen": start_fen,
        ...
        for move_stat in data["moves"]:
            ...
            assert move_stat["total_games"] == move_stat["white"] + move_stat["black"] + move_stat["draw"]
...
class TestHistoricalEndpoint:
    ...
    async def test_historical_returns_most_popular(self, client: AsyncClient):
        ...
        moves = [
            {"move": "e4e5", "end_fen": "rnbqkb1r/pppppppp/5n2/4P3/8/8/PPPP1PPP/RNBQKBNR b KQkq -", "white": 100, "black": 50, "draw": 25},
            {"move": "b1c3", "end_fen": "rnbqkb1r/pppppppp/5n2/8/4P3/2N5/PPPP1PPP/R1BQKBNR b KQkq -", "white": 500, "black": 300, "draw": 200},  # Most popular
            {"move": "d2d3", "end_fen": "rnbqkb1r/pppppppp/5n2/8/4P3/3P4/PPP2PPP/RNBQKBNR b KQkq -", "white": 50, "black": 25, "draw": 10},
        ]
...
class TestDatabasePersistence:
    ...
    async def test_move_persists_across_requests(self, client: AsyncClient):
        ...
        # Create move
        create_response = await client.post("/moves/", json={
            "move": "e1g1",  # Castling
            "start_fen": start_fen,
            "end_fen": end_fen,
            "white": 750,
    ...
    async def test_multiple_moves_same_position(self, client: AsyncClient):
        ...
        moves = [
            {"move": "g1f3", "end_fen": "rnbqkbnr/pppp1ppp/8/4p3/4P3/5N2/PPPP1PPP/RNBQKB1R b KQkq -"},
            {"move": "b1c3", "end_fen": "rnbqkbnr/pppp1ppp/8/4p3/4P3/2N5/PPPP1PPP/R1BQKBNR b KQkq -"},
            {"move": "f1c4", "end_fen": "rnbqkbnr/pppp1ppp/8/4p3/2B1P3/8/PPPP1PPP/RNBQK1NR b KQkq -"},
        ]
